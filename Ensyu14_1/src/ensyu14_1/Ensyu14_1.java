...
public class Ensyu14_1 {
	...
	public static Shape[] inputFigure(int selectNumber, int indexNumber, Shape[] arrayNumber) {
		...
		switch (selectNumber) {
			...
			case 5:
				...
				System.out.print("幅：");
				//幅をキーボードから読み込む
				int widthNumberTwo = oneNumber();
				//高さの入力を促す
				System.out.print("高さ：");
				//高さをキーボードから読み込む
				int heightNumberTwo = oneNumber();
				//arrayNumber[indexNumber]にインスタンス化する
				arrayNumber[indexNumber] = new Parallelogram(widthNumberTwo, heightNumberTwo);
				//switch文から抜ける
				break;
	...
	public static void main(String[] args) {
		//図形の種類数を表す変数FIGURE_KIND_NUMBERを生成し、5で初期化する
		final int FIGURE_KIND_NUMBER = 5;
		...
		for (int i = 0; i < figureNumber; i++) {
			...
			do {
				...
				//選べる図形の種類を表示する
				System.out.print("(1…点/2…水平直線/3…垂直直線/4…長方形/5…平行四辺形)：");
				//図形の種類をキーボードから読み込む
		...
		for (int i = 0; i < figureNumber; i++) {
			...
			//面積を求められる図形の場合
			if (shapeArray[i] instanceof Plane2D) {
				//図形の面積を表示する
				System.out.println("面積：" + ((Plane2D)shapeArray[i]).getArea());
			}
			//改行する
			System.out.println();
		}
